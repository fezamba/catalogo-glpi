document.addEventListener('DOMContentLoaded', () => {
  ...
  const getBotResponse = async (userMessage, fullContext) => {
    const historyForPrompt = conversationHistory
      .map(
        (turn) =>
          `  - ${turn.role === 'user' ? 'Usuário' : 'Assistente'}: "${
            turn.parts[0].text
          }"`
      )
      .join('\n');
        ...
        const prompt = `
            ...
            **4. CONSULTA ÀS FICHAS DE SERVIÇO (BASE DE CONHECIMENTO)**
            ...
            - **INSTRUÇÃO DE ESCALONAMENTO:** Após descrever a ficha, adicione a frase:
                "Este é um serviço que deve ser solicitado via chamado. Para registar, por favor, acesse o link <a href='link-service-desk' target='_blank'>Service Desk</a> e mencione o código da ficha."
